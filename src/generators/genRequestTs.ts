...
export function generateTsRequestFile(operation: ParsedOperation, httpClientPath: string, typeImportPath: string, mergeParams: boolean = false): string {
  const importTypes = [
...
${importLine}
${buildUrlHelper}
${renderFunction(operation, false, mergeParams)}
`;
}

export function generateTsModuleFile(operations: ParsedOperation[], httpClientPath: string, typeImportPath: string, mergeParams: boolean = false): string {
...
`
  ...
  const functions = operations.map((op) => renderFunction(op, true, mergeParams)).join('\n\n');
